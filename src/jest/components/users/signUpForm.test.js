...
describe('Test case for testing signUp',() =>{
  ...
  it('input check', () => {
    ...
    component.find('input[type="text"]').simulate('change', {target: {name: 'username', value: 'username'}});
    expect(component.state('username')).toEqual('username');
    component.find('input[type="text"]').simulate('change', {target: {name: 'first_name', value: 'fname'}});
    expect(component.state('first_name')).toEqual('fname');
    component.find('input[type="text"]').simulate('change', {target: {name: 'last_name', value: 'lname'}});
